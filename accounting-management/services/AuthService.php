<?php
namespace AccountingManagementApi\Services;
...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action, string $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'acc_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'accounting_sales_executive');

        // Validate role is allowed
        $allowed_roles = ['accounting_super_admin', 'accounting_accountant', 'accounting_sales_executive', 'accounting_purchase_manager', 'accounting_auditor'];
        ...
        $transient_key = 'accounting_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $subject = get_option('accounting_email_subject', 'Accounting ERP Verification Code');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'accounting_super_admin') ? 'APPROVED' : 'PENDING';
        update_user_meta($user_id, 'accounting_user_status', $status);
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['asuperadmin', 'aaccountant', 'asalesexec', 'apurchasemgr', 'aauditor'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'accounting_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'accounting_super_admin', 'accounting_accountant', 'accounting_sales_executive', 'accounting_purchase_manager', 'accounting_auditor'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Accounting ERP Portal");
        ...
        update_user_meta($user->ID, 'accounting_refresh_token', $refresh_token);

        $status = get_user_meta($user->ID, 'accounting_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'accounting_refresh_token', true);
        ...
        update_user_meta($user_id, 'accounting_refresh_token', $new_refresh);
