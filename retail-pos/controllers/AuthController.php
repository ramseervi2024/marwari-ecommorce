...
class AuthController extends BaseController {
    ...
    public function me(WP_REST_Request $request) {
        ...
        $status = get_user_meta($user_id, 'pos_user_status', true) ?: 'APPROVED';
    ...
    public function logout(WP_REST_Request $request) {
        $user_id = get_current_user_id();
        delete_user_meta($user_id, 'pos_refresh_token');
    ...
    public function getUsers(WP_REST_Request $request) {
        $users = get_users([
            'role__in' => ['pos_store_manager', 'pos_cashier', 'pos_inventory_manager']
        ]);

        $formatted = [];
        foreach ($users as $user) {
            $status = get_user_meta($user->ID, 'pos_user_status', true) ?: 'PENDING';
            $formatted[] = [
                'id' => $user->ID,
                'username' => $user->user_login,
                'email' => $user->user_email,
    ...
    public function updateUserStatus(WP_REST_Request $request) {
        ...
        update_user_meta($user_id, 'pos_user_status', $status);
    ...
    public function getSmtpSettings(WP_REST_Request $request) {
        return $this->success('Email settings retrieved successfully.', [
            ...
            'from_name' => get_option('retail_smtp_from_name', 'Global Retail POS'),
            'subject' => get_option('retail_email_subject', 'Retail POS Verification Code'),
            'template' => get_option('retail_email_template', "Hello {name},\n\nYour 6-digit verification code is: {otp}\n\nThis code is valid for 15 minutes.\n\nThank you!"),
            'smtp_enabled' => get_option('retail_smtp_enabled', 'no'),
            'smtp_host' => get_option('retail_smtp_host', ''),
            'smtp_port' => get_option('retail_smtp_port', '587'),
            'smtp_username' => get_option('retail_smtp_username', ''),
            'smtp_password' => get_option('retail_smtp_password', ''),
            'smtp_encryption' => get_option('retail_smtp_encryption', 'tls')
        ]);
    }
    ...
    public function saveSmtpSettings(WP_REST_Request $request) {
        ...
        update_option('retail_smtp_from_email', sanitize_email($params['from_email'] ?? ''));
        update_option('retail_smtp_from_name', sanitize_text_field($params['from_name'] ?? ''));
        update_option('retail_email_subject', sanitize_text_field($params['subject'] ?? ''));
        update_option('retail_email_template', sanitize_textarea_field($template));
        update_option('retail_smtp_enabled', sanitize_text_field($params['smtp_enabled'] ?? 'no'));
        update_option('retail_smtp_host', sanitize_text_field($params['smtp_host'] ?? ''));
        update_option('retail_smtp_port', sanitize_text_field($params['smtp_port'] ?? '587'));
        update_option('retail_smtp_username', sanitize_text_field($params['smtp_username'] ?? ''));
        update_option('retail_smtp_password', sanitize_text_field($params['smtp_password'] ?? ''));
        update_option('retail_smtp_encryption', sanitize_text_field($params['smtp_encryption'] ?? 'tls'));
    ...
    public function testSmtpSettings(WP_REST_Request $request) {
        ...
        $subject = 'Retail POS Test Mail';
        $message = "This is a test email from your Global Retail POS email settings configuration.\n\nIf you are reading this message, your email delivery settings are working successfully!";
