...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action, string $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'pos_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'pos_cashier');

        // Validate role is allowed
        $allowed_roles = ['pos_super_admin', 'pos_store_manager', 'pos_cashier', 'pos_inventory_manager'];
        ...
        $transient_key = 'retail_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $smtp_from_email = get_option('retail_smtp_from_email');
        $smtp_from_name = get_option('retail_smtp_from_name', 'Global Retail POS');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'pos_super_admin') ? 'APPROVED' : 'PENDING';
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['possuperadmin', 'pos_manager', 'pos_cashier', 'pos_inventory'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'retail_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'pos_super_admin', 'pos_store_manager', 'pos_cashier', 'pos_inventory_manager'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Retail POS Portal");
        ...
        update_user_meta($user->ID, 'pos_refresh_token', $refresh_token);

        $status = get_user_meta($user->ID, 'pos_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'pos_refresh_token', true);
        ...
        update_user_meta($user_id, 'pos_refresh_token', $new_refresh);
