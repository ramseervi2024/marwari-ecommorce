<?php
namespace TransportManagementApi\Services;
...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action, string $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'transport_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'transport_driver');

        // Validate role is allowed
        $allowed_roles = ['transport_super_admin', 'transport_fleet_manager', 'transport_operations_manager', 'transport_driver', 'transport_accountant'];
        ...
        $transient_key = 'transport_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $subject = get_option('transport_email_subject', 'Transport ERP Verification Code');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'transport_super_admin') ? 'APPROVED' : 'PENDING';
        update_user_meta($user_id, 'transport_user_status', $status);

        // Add to custom drivers table if role is driver
        global $wpdb;
        if ($role === 'transport_driver') {
            $table_drivers = $wpdb->prefix . 'transport_drivers';
            $driver_code = 'DRV-' . date('Ymd') . sprintf('%03d', rand(1, 999));
            $wpdb->insert($table_drivers, [
                'driver_code' => $driver_code,
                'name' => $name,
                'mobile' => '9999999999',
                'license_number' => 'DL-PENDING',
                'license_expiry' => date('Y-m-d', strtotime('+3 years')),
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['tsuperadmin', 'tfleetmgr', 'topsmgr', 'tdriver1', 'taccountant'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'transport_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'transport_super_admin', 'transport_fleet_manager', 'transport_operations_manager', 'transport_driver', 'transport_accountant'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Transport ERP Portal");
        ...
        update_user_meta($user->ID, 'transport_refresh_token', $refresh_token);

        $status = get_user_meta($user->ID, 'transport_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'transport_refresh_token', true);
        ...
        update_user_meta($user_id, 'transport_refresh_token', $new_refresh);
