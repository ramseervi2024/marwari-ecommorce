<?php
namespace FleetTrackPro\Services;
...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action, string $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'fleet_activity_logs';
    ...
    /**
     * Register a new user and assign custom fleet role
     */
    public function register(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'fleet_driver');

        // Validate role is allowed
        $allowed_roles = ['fleet_super_admin', 'fleet_manager', 'fleet_accountant', 'fleet_driver'];
    ...
    public function login(string $username, string $password) {
        ...
        // Check if user has fleet role or admin role
        $role = !empty($user->roles) ? $user->roles[0] : '';
        $allowed_roles = ['administrator', 'fleet_super_admin', 'fleet_manager', 'fleet_accountant', 'fleet_driver'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for FleetTrack Pro");
        ...
        update_user_meta($user->ID, 'fleet_refresh_token', $refresh_token);
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'fleet_refresh_token', true);
        ...
        update_user_meta($user_id, 'fleet_refresh_token', $new_refresh);
