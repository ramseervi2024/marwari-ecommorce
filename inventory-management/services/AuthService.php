...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action, string $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'inv_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'inventory_warehouse_staff');

        // Validate role is allowed
        $allowed_roles = ['inventory_super_admin', 'inventory_manager', 'inventory_purchase_manager', 'inventory_warehouse_staff', 'inventory_auditor'];
        ...
        $transient_key = 'inventory_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $subject = get_option('inventory_email_subject', 'Inventory ERP Verification Code');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'inventory_super_admin') ? 'APPROVED' : 'PENDING';
        update_user_meta($user_id, 'inventory_user_status', $status);
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['isuperadmin', 'imanager', 'ipurchasemgr', 'istaff', 'iauditor'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'inventory_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'inventory_super_admin', 'inventory_manager', 'inventory_purchase_manager', 'inventory_warehouse_staff', 'inventory_auditor'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Inventory ERP Portal");
        ...
        update_user_meta($user->ID, 'inventory_refresh_token', $refresh_token);

        $status = get_user_meta($user->ID, 'inventory_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'inventory_refresh_token', true);
        ...
        update_user_meta($user_id, 'inventory_refresh_token', $new_refresh);
