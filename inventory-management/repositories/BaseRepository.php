...
class BaseRepository {
    ...
    public function __construct(string $table_suffix, bool $has_soft_delete = true) {
        global $wpdb;
        $this->table_name = $wpdb->prefix . 'inv_' . $table_suffix;
    ...
    public function exists(string $column, string $value, ?int $exclude_id = null): bool {
        global $wpdb;
        $query = "SELECT COUNT(*) FROM {$this->table_name} WHERE {$column} = %s";
        if ($this->has_soft_delete) {
            $query .= " AND deleted_at IS NULL";
        }
        $args = [$value];
