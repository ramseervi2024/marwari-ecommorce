...
class AuthController extends BaseController {
    ...
    public function me(WP_REST_Request $request) {
        ...
        $status = get_user_meta($user_id, 'inventory_user_status', true) ?: 'APPROVED';
    ...
    public function logout(WP_REST_Request $request) {
        $user_id = get_current_user_id();
        delete_user_meta($user_id, 'inventory_refresh_token');
    ...
    public function getSmtpSettings(WP_REST_Request $request) {
        return $this->success('SMTP settings retrieved.', [
            ...
            'smtp_host' => get_option('inventory_smtp_host', ''),
            'smtp_port' => get_option('inventory_smtp_port', '587'),
            'smtp_username' => get_option('inventory_smtp_username', ''),
            'smtp_encryption' => get_option('inventory_smtp_encryption', 'tls'),
            'smtp_from_email' => get_option('inventory_smtp_from_email', ''),
            'smtp_from_name' => get_option('inventory_smtp_from_name', 'Inventory ERP')
        ]);
    }
    ...
    public function saveSmtpSettings(WP_REST_Request $request) {
        ...
        update_option('inventory_smtp_enabled', sanitize_text_field($params['smtp_enabled'] ?? 'no'));
        update_option('inventory_smtp_host', sanitize_text_field($params['smtp_host'] ?? ''));
        update_option('inventory_smtp_port', sanitize_text_field($params['smtp_port'] ?? '587'));
        update_option('inventory_smtp_username', sanitize_text_field($params['smtp_username'] ?? ''));
        if (isset($params['smtp_password']) && $params['smtp_password'] !== '******') {
            update_option('inventory_smtp_password', sanitize_text_field($params['smtp_password']));
        }
        update_option('inventory_smtp_encryption', sanitize_text_field($params['smtp_encryption'] ?? 'tls'));
        update_option('inventory_smtp_from_email', sanitize_email($params['smtp_from_email'] ?? ''));
        update_option('inventory_smtp_from_name', sanitize_text_field($params['smtp_from_name'] ?? 'Inventory ERP'));
    ...
    public function testSmtpSettings(WP_REST_Request $request) {
        ...
        $subject = 'Inventory ERP - SMTP Connection Test';
        $message = 'This is a test email from your Inventory Management ERP SMTP configuration. If you receive this, your SMTP settings are working perfectly!';
    ...
    public function getUsers(WP_REST_Request $request) {
        $users = get_users([
            'role__in' => ['inventory_super_admin', 'inventory_manager', 'inventory_purchase_manager', 'inventory_warehouse_staff', 'inventory_auditor']
        ]);
        ...
        foreach ($users as $u) {
            $list[] = [
                ...
                'status' => get_user_meta($u->ID, 'inventory_user_status', true) ?: 'APPROVED'
            ];
        }
    ...
    public function updateUserStatus(WP_REST_Request $request) {
        ...
        update_user_meta($uid, 'inventory_user_status', $status);
