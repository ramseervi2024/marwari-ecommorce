<?php
namespace ManufacturingManagementApi\Services;
...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action_type, string $description = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'mfg_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'mfg_store_manager');

        $allowed_roles = ['mfg_super_admin', 'mfg_production_manager', 'mfg_purchase_manager', 'mfg_store_manager', 'mfg_quality_inspector', 'mfg_dispatch_manager'];
        ...
        $transient_key = 'mfg_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $smtp_from_email = get_option('mfg_smtp_from_email');
        $smtp_from_name = get_option('mfg_smtp_from_name', 'Global Manufacturing ERP');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'mfg_super_admin') ? 'APPROVED' : 'PENDING';
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['mfgsuperadmin', 'mfg_production', 'mfg_purchase', 'mfg_store', 'mfg_quality', 'mfg_dispatch'];
        ...
        $otp = strval(rand(100000, 999999));
        $transient_key = 'mfg_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'mfg_super_admin', 'mfg_production_manager', 'mfg_purchase_manager', 'mfg_store_manager', 'mfg_quality_inspector', 'mfg_dispatch_manager'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Manufacturing ERP Portal");
        ...
        update_user_meta($user->ID, 'mfg_refresh_token', $refresh_token);
        $status = get_user_meta($user->ID, 'mfg_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'mfg_refresh_token', true);
        ...
        update_user_meta($user_id, 'mfg_refresh_token', $new_refresh);
        $status = get_user_meta($user->ID, 'mfg_user_status', true) ?: 'APPROVED';
