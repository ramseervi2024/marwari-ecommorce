<?php
namespace JewelleryManagementApi\Services;
...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action_type, string $description = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'jewel_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'jewel_sales_executive');

        $allowed_roles = ['jewel_super_admin', 'jewel_store_manager', 'jewel_sales_executive', 'jewel_karigar_supervisor', 'jewel_accountant'];
        ...
        $transient_key = 'jwl_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $smtp_from_email = get_option('jwl_smtp_from_email');
        $smtp_from_name = get_option('jwl_smtp_from_name', 'Jewellery ERP');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'jewel_super_admin') ? 'APPROVED' : 'PENDING';
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['jewelsuperadmin', 'jwl_manager', 'jwl_sales', 'jwl_supervisor', 'jwl_accountant'];
        ...
        $otp = strval(rand(100000, 999999));
        $transient_key = 'jwl_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'jewel_super_admin', 'jewel_store_manager', 'jewel_sales_executive', 'jewel_karigar_supervisor', 'jewel_accountant'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Jewellery ERP Portal");
        ...
        update_user_meta($user->ID, 'jwl_refresh_token', $refresh_token);
        $status = get_user_meta($user->ID, 'jewel_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'jwl_refresh_token', true);
        ...
        update_user_meta($user_id, 'jwl_refresh_token', $new_refresh);
        $status = get_user_meta($user->ID, 'jewel_user_status', true) ?: 'APPROVED';
