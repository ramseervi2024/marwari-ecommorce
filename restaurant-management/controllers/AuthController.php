...
use RestaurantManagementApi\Services\AuthService;
use WP_REST_Request;
...
class AuthController extends BaseController {
    ...
    public function me(WP_REST_Request $request) {
        ...
        $status = get_user_meta($user_id, 'restaurant_user_status', true) ?: 'APPROVED';
    ...
    public function logout(WP_REST_Request $request) {
        $user_id = get_current_user_id();
        delete_user_meta($user_id, 'restaurant_refresh_token');
    ...
    public function getUsers(WP_REST_Request $request) {
        $users = get_users([
            'role__in' => ['restaurant_manager', 'restaurant_cashier', 'restaurant_chef', 'restaurant_waiter', 'restaurant_delivery']
        ]);

        $formatted = [];
        foreach ($users as $user) {
            $status = get_user_meta($user->ID, 'restaurant_user_status', true) ?: 'PENDING';
            $formatted[] = [
                'id' => $user->ID,
                'username' => $user->user_login,
                'email' => $user->user_email,
    ...
    public function getSmtpSettings(WP_REST_Request $request) {
        return $this->success('Email settings retrieved successfully.', [
            ...
            'from_name' => get_option('restaurant_smtp_from_name', 'Global Restaurant POS'),
            'subject' => get_option('restaurant_email_subject', 'Restaurant ERP Verification Code'),
            'template' => get_option('restaurant_email_template', "Hello {name},\n\nYour 6-digit verification code is: {otp}\n\nThis code is valid for 15 minutes.\n\nThank you!"),
            'smtp_enabled' => get_option('restaurant_smtp_enabled', 'no'),
            'smtp_host' => get_option('restaurant_smtp_host', ''),
            'smtp_port' => get_option('restaurant_smtp_port', '587'),
            'smtp_username' => get_option('restaurant_smtp_username', ''),
            'smtp_password' => get_option('restaurant_smtp_password', ''),
            'smtp_encryption' => get_option('restaurant_smtp_encryption', 'tls')
        ]);
    }
    ...
    public function saveSmtpSettings(WP_REST_Request $request) {
        ...
        update_option('restaurant_smtp_from_email', sanitize_email($params['from_email'] ?? ''));
        update_option('restaurant_smtp_from_name', sanitize_text_field($params['from_name'] ?? ''));
        update_option('restaurant_email_subject', sanitize_text_field($params['subject'] ?? ''));
        update_option('restaurant_email_template', sanitize_textarea_field($template));
        update_option('restaurant_smtp_enabled', sanitize_text_field($params['smtp_enabled'] ?? 'no'));
        update_option('restaurant_smtp_host', sanitize_text_field($params['smtp_host'] ?? ''));
        update_option('restaurant_smtp_port', sanitize_text_field($params['smtp_port'] ?? '587'));
        update_option('restaurant_smtp_username', sanitize_text_field($params['smtp_username'] ?? ''));
        update_option('restaurant_smtp_password', sanitize_text_field($params['smtp_password'] ?? ''));
        update_option('restaurant_smtp_encryption', sanitize_text_field($params['smtp_encryption'] ?? 'tls'));
    ...
    public function testSmtpSettings(WP_REST_Request $request) {
        ...
        $subject = 'Restaurant POS Test Mail';
        $message = "This is a test email from your Global Restaurant POS email settings configuration.\n\nIf you are reading this message, your email delivery settings are working successfully!";
