...
class AuthController extends BaseController {
    ...
    public function me(WP_REST_Request $request) {
        ...
        $status = get_user_meta($user_id, 'crm_user_status', true) ?: 'APPROVED';
    ...
    public function logout(WP_REST_Request $request) {
        $user_id = get_current_user_id();
        delete_user_meta($user_id, 'crm_refresh_token');
    ...
    public function getSmtpSettings(WP_REST_Request $request) {
        return $this->success('SMTP settings retrieved.', [
            ...
            'smtp_host' => get_option('crm_smtp_host', ''),
            'smtp_port' => get_option('crm_smtp_port', '587'),
            'smtp_username' => get_option('crm_smtp_username', ''),
            'smtp_encryption' => get_option('crm_smtp_encryption', 'tls'),
    ...
    public function saveSmtpSettings(WP_REST_Request $request) {
        ...
        update_option('crm_smtp_enabled', sanitize_text_field($params['smtp_enabled'] ?? 'no'));
        update_option('crm_smtp_host', sanitize_text_field($params['smtp_host'] ?? ''));
        update_option('crm_smtp_port', sanitize_text_field($params['smtp_port'] ?? '587'));
        update_option('crm_smtp_username', sanitize_text_field($params['smtp_username'] ?? ''));
        if (isset($params['smtp_password']) && $params['smtp_password'] !== '******') {
            update_option('crm_smtp_password', sanitize_text_field($params['smtp_password']));
        }
        update_option('crm_smtp_encryption', sanitize_text_field($params['smtp_encryption'] ?? 'tls'));
        update_option('crm_smtp_from_email', sanitize_email($params['smtp_from_email'] ?? ''));
        update_option('crm_smtp_from_name', sanitize_text_field($params['smtp_from_name'] ?? 'CRM ERP'));
    ...
    public function testSmtpSettings(WP_REST_Request $request) {
        ...
        $subject = 'CRM ERP - SMTP Connection Test';
        $message = 'This is a test email from your CRM ERP SMTP configuration. If you receive this, your SMTP settings are working perfectly!';
    ...
    public function getUsers(WP_REST_Request $request) {
        $users = get_users([
            'role__in' => ['crm_super_admin', 'crm_sales_manager', 'crm_sales_executive', 'crm_telecaller', 'crm_customer']
        ]);
        ...
        foreach ($users as $u) {
            $list[] = [
                ...
                'status' => get_user_meta($u->ID, 'crm_user_status', true) ?: 'APPROVED'
            ];
        }
    ...
    public function updateUserStatus(WP_REST_Request $request) {
        ...
        update_user_meta($uid, 'crm_user_status', $status);
    ...
    public function deleteUser(WP_REST_Request $request) {
        ...
        // Clean up from CRM custom tables
        global $wpdb;
        $wpdb->delete($wpdb->prefix . 'crm_customers', ['user_id' => $id]);
