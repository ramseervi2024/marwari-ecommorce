...
class AuthService {
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'crm_sales_executive');

        // Validate role is allowed
        $allowed_roles = ['crm_super_admin', 'crm_sales_manager', 'crm_sales_executive', 'crm_telecaller', 'crm_customer'];
        ...
        $transient_key = 'crm_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $subject = get_option('crm_email_subject', 'CRM ERP Verification Code');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'crm_super_admin') ? 'APPROVED' : 'PENDING';
        update_user_meta($user_id, 'crm_user_status', $status);

        // If registered as crm_customer, create record in wp_crm_customers
        if ($role === 'crm_customer') {
            global $wpdb;
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['csuperadmin', 'cmanager', 'cexecutive', 'ctelecaller', 'ccustomer'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'crm_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'crm_super_admin', 'crm_sales_manager', 'crm_sales_executive', 'crm_telecaller', 'crm_customer'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for CRM ERP Portal");
        ...
        update_user_meta($user->ID, 'crm_refresh_token', $refresh_token);
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'crm_refresh_token', true);
        ...
        update_user_meta($user_id, 'crm_refresh_token', $new_refresh);
