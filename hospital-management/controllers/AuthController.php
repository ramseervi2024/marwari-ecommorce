...
class AuthController extends BaseController {
    ...
    public function login(WP_REST_Request $request) {
        ...
        if (!empty($params['password'])) {
            // Password based login (seeded / admin accounts)
            $result = $this->authService->login($params['username'], $params['password']);
        } else if (!empty($params['otp'])) {
            // OTP based passwordless login (standard users)
    ...
    public function me(WP_REST_Request $request) {
        ...
        $status = get_user_meta($user_id, 'hospital_user_status', true) ?: 'APPROVED';
    ...
    public function logout(WP_REST_Request $request) {
        $user_id = get_current_user_id();
        delete_user_meta($user_id, 'hospital_refresh_token');
        AuthService::logActivity($user_id, 'LOGOUT', 'User signed out from the portal');
        return $this->success('Signed out successfully.');
    }

    /**
     * GET /auth/users (approvals grid for Super Admin)
     */
    public function getUsers(WP_REST_Request $request) {
        $users = get_users([
            'role__in' => ['hospital_doctor', 'hospital_receptionist', 'hospital_pharmacist', 'hospital_lab_technician', 'hospital_patient']
        ]);

        $formatted = [];
        foreach ($users as $user) {
            $status = get_user_meta($user->ID, 'hospital_user_status', true) ?: 'PENDING';
            $formatted[] = [
                'id' => $user->ID,
                'username' => $user->user_login,
                'email' => $user->user_email,
    ...
    /**
     * POST /auth/users/status (Approve/Hold/Block user)
     */
    public function updateUserStatus(WP_REST_Request $request) {
        ...
        update_user_meta($user_id, 'hospital_user_status', $status);
    ...
    public function deleteUser(WP_REST_Request $request) {
        ...
        // Also delete from custom patient or doctor tables if applicable
        global $wpdb;
        $wpdb->update($wpdb->prefix . 'hospital_patients', ['deleted_at' => current_time('mysql')], ['email' => $user->user_email]);
        $wpdb->update($wpdb->prefix . 'hospital_doctors', ['deleted_at' => current_time('mysql')], ['email' => $user->user_email]);
    ...
    public function getSmtpSettings(WP_REST_Request $request) {
        return $this->success('Email settings retrieved successfully.', [
            'from_email' => get_option('hospital_smtp_from_email', 'rameshseervi242628@gmail.com'),
            'from_name' => get_option('hospital_smtp_from_name', 'Global Hospital ERP'),
            'subject' => get_option('hospital_email_subject', 'Hospital ERP Verification Code'),
            'template' => get_option('hospital_email_template', "Hello {name},\n\nYour 6-digit verification code is: {otp}\n\nThis code is valid for 15 minutes.\n\nThank you!"),
            'smtp_enabled' => get_option('hospital_smtp_enabled', 'no'),
            'smtp_host' => get_option('hospital_smtp_host', ''),
            'smtp_port' => get_option('hospital_smtp_port', '587'),
            'smtp_username' => get_option('hospital_smtp_username', ''),
            'smtp_password' => get_option('hospital_smtp_password', ''),
            'smtp_encryption' => get_option('hospital_smtp_encryption', 'tls')
        ]);
    }
    ...
    public function saveSmtpSettings(WP_REST_Request $request) {
        ...
        update_option('hospital_smtp_from_email', sanitize_email($params['from_email'] ?? ''));
        update_option('hospital_smtp_from_name', sanitize_text_field($params['from_name'] ?? ''));
        update_option('hospital_email_subject', sanitize_text_field($params['subject'] ?? ''));
        update_option('hospital_email_template', sanitize_textarea_field($template));
        update_option('hospital_smtp_enabled', sanitize_text_field($params['smtp_enabled'] ?? 'no'));
        update_option('hospital_smtp_host', sanitize_text_field($params['smtp_host'] ?? ''));
        update_option('hospital_smtp_port', sanitize_text_field($params['smtp_port'] ?? '587'));
        update_option('hospital_smtp_username', sanitize_text_field($params['smtp_username'] ?? ''));
        update_option('hospital_smtp_password', sanitize_text_field($params['smtp_password'] ?? ''));
        update_option('hospital_smtp_encryption', sanitize_text_field($params['smtp_encryption'] ?? 'tls'));
    ...
    public function testSmtpSettings(WP_REST_Request $request) {
        ...
        $subject = 'Hospital ERP Test Mail';
        $message = "This is a test email from your Global Hospital ERP email settings configuration.\n\nIf you are reading this message, your email delivery settings are working successfully!";
