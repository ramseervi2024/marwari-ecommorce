...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action, string $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'hospital_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'hospital_patient');

        // Validate role is allowed
        $allowed_roles = ['hospital_super_admin', 'hospital_doctor', 'hospital_receptionist', 'hospital_pharmacist', 'hospital_lab_technician', 'hospital_patient'];
        ...
        $transient_key = 'hospital_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $smtp_from_email = get_option('hospital_smtp_from_email');
        $smtp_from_name = get_option('hospital_smtp_from_name', 'Global Hospital ERP');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'hospital_super_admin') ? 'APPROVED' : 'PENDING';
        update_user_meta($user_id, 'hospital_user_status', $status);

        // Additionally insert into custom patient or doctor tables if applicable
        global $wpdb;
        if ($role === 'hospital_patient') {
            $table_patients = $wpdb->prefix . 'hospital_patients';
            $patient_code = 'PAT' . date('Ymd') . sprintf('%03d', rand(1, 999));
        ...
        } elseif ($role === 'hospital_doctor') {
            $table_doctors = $wpdb->prefix . 'hospital_doctors';
            $doctor_code = 'DOC' . date('Ymd') . sprintf('%03d', rand(1, 999));
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['hospitalsuperadmin', 'hospital_doctor', 'hospital_receptionist', 'hospital_pharmacist', 'hospital_lab_technician', 'hospital_patient'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'hospital_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'hospital_super_admin', 'hospital_doctor', 'hospital_receptionist', 'hospital_pharmacist', 'hospital_lab_technician', 'hospital_patient'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Hospital ERP Portal");
        ...
        update_user_meta($user->ID, 'hospital_refresh_token', $refresh_token);

        $status = get_user_meta($user->ID, 'hospital_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'hospital_refresh_token', true);
        ...
        update_user_meta($user_id, 'hospital_refresh_token', $new_refresh);
