...
class AuthController extends BaseController {
    ...
    public function me(WP_REST_Request $request) {
        ...
        $status = get_user_meta($user_id, 'garment_user_status', true) ?: 'APPROVED';
    ...
    public function logout(WP_REST_Request $request) {
        $user_id = get_current_user_id();
        delete_user_meta($user_id, 'gmt_refresh_token');
    ...
    public function getUsers(WP_REST_Request $request) {
        $users = get_users([
            'role__in' => ['garment_production_manager', 'garment_inventory_manager', 'garment_supervisor', 'garment_quality_inspector', 'garment_dispatch_manager']
        ]);

        $formatted = [];
        foreach ($users as $user) {
            $status = get_user_meta($user->ID, 'garment_user_status', true) ?: 'PENDING';
            $formatted[] = [
                'id' => $user->ID,
                'username' => $user->user_login,
                'email' => $user->user_email,
    ...
    public function updateUserStatus(WP_REST_Request $request) {
        ...
        update_user_meta($user_id, 'garment_user_status', $status);
    ...
    public function getSmtpSettings(WP_REST_Request $request) {
        return $this->success('Email settings retrieved successfully.', [
            ...
            'from_name' => get_option('gmt_smtp_from_name', 'Garment Textile ERP'),
            'subject' => get_option('gmt_email_subject', 'Garment ERP Verification Code'),
            'template' => get_option('gmt_email_template', "Hello {name},\n\nYour 6-digit verification code is: {otp}\n\nThis code is valid for 15 minutes.\n\nThank you!"),
            'smtp_enabled' => get_option('gmt_smtp_enabled', 'no'),
            'smtp_host' => get_option('gmt_smtp_host', ''),
            'smtp_port' => get_option('gmt_smtp_port', '587'),
            'smtp_username' => get_option('gmt_smtp_username', ''),
            'smtp_password' => get_option('gmt_smtp_password', ''),
            'smtp_encryption' => get_option('gmt_smtp_encryption', 'tls')
        ]);
    }
    ...
    public function saveSmtpSettings(WP_REST_Request $request) {
        ...
        update_option('gmt_smtp_from_email', sanitize_email($params['from_email'] ?? ''));
        update_option('gmt_smtp_from_name', sanitize_text_field($params['from_name'] ?? ''));
        update_option('gmt_email_subject', sanitize_text_field($params['subject'] ?? ''));
        update_option('gmt_email_template', sanitize_textarea_field($template));
        update_option('gmt_smtp_enabled', sanitize_text_field($params['smtp_enabled'] ?? 'no'));
        update_option('gmt_smtp_host', sanitize_text_field($params['smtp_host'] ?? ''));
        update_option('gmt_smtp_port', sanitize_text_field($params['smtp_port'] ?? '587'));
        update_option('gmt_smtp_username', sanitize_text_field($params['smtp_username'] ?? ''));
        update_option('gmt_smtp_password', sanitize_text_field($params['smtp_password'] ?? ''));
        update_option('gmt_smtp_encryption', sanitize_text_field($params['smtp_encryption'] ?? 'tls'));
    ...
    public function testSmtpSettings(WP_REST_Request $request) {
        ...
        $subject = 'Garment ERP Test Mail';
        $message = "This is a test email from your Garment Textile ERP email settings configuration.\n\nIf you are reading this message, your email delivery settings are working successfully!";
