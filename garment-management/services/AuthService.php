<?php
namespace GarmentManagementApi\Services;
...
class AuthService {
     ...
     */
    public static function logActivity(?int $user_id, string $action_type, string $description = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'garment_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'garment_inventory_manager');

        $allowed_roles = ['garment_super_admin', 'garment_production_manager', 'garment_inventory_manager', 'garment_supervisor', 'garment_quality_inspector', 'garment_dispatch_manager'];
        ...
        $transient_key = 'gmt_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $smtp_from_email = get_option('gmt_smtp_from_email');
        $smtp_from_name = get_option('gmt_smtp_from_name', 'Garment Textile ERP');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'garment_super_admin') ? 'APPROVED' : 'PENDING';
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['garmentsuperadmin', 'gmt_production', 'gmt_inventory', 'gmt_supervisor', 'gmt_quality', 'gmt_dispatch'];
        ...
        $otp = strval(rand(100000, 999999));
        $transient_key = 'gmt_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'garment_super_admin', 'garment_production_manager', 'garment_inventory_manager', 'garment_supervisor', 'garment_quality_inspector', 'garment_dispatch_manager'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Garment ERP Portal");
        ...
        update_user_meta($user->ID, 'gmt_refresh_token', $refresh_token);
        $status = get_user_meta($user->ID, 'garment_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'gmt_refresh_token', true);
        ...
        update_user_meta($user_id, 'gmt_refresh_token', $new_refresh);
        $status = get_user_meta($user->ID, 'garment_user_status', true) ?: 'APPROVED';
