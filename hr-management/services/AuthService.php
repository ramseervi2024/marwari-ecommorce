...
class AuthService {
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'hr_employee');

        // Validate role is allowed
        $allowed_roles = ['hr_super_admin', 'hr_manager', 'hr_accountant', 'hr_employee'];
        ...
        $transient_key = 'hr_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $subject = get_option('hr_email_subject', 'HR ERP Verification Code');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'hr_super_admin') ? 'APPROVED' : 'PENDING';
        update_user_meta($user_id, 'hr_user_status', $status);

        // Also create a record in wp_hr_employees
        global $wpdb;
        ...
        $wpdb->insert($table_employees, [
            ...
            'date_of_joining' => current_time('mysql', 1),
            'pf_number' => '',
            'esi_number' => '',
            'status' => 'ACTIVE'
        ]);
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['hsuperadmin', 'hmanager', 'haccountant', 'hemployee'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'hr_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'hr_super_admin', 'hr_manager', 'hr_accountant', 'hr_employee'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for HR ERP Portal");
        ...
        update_user_meta($user->ID, 'hr_refresh_token', $refresh_token);

        $status = get_user_meta($user->ID, 'hr_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'hr_refresh_token', true);
        ...
        update_user_meta($user_id, 'hr_refresh_token', $new_refresh);
