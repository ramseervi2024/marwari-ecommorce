<?php
namespace ConstructionManagementApi\Services;
...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action, string $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'construction_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'construction_contractor');

        // Validate role is allowed
        $allowed_roles = ['construction_super_admin', 'construction_project_manager', 'construction_site_engineer', 'construction_purchase_manager', 'construction_contractor', 'construction_accountant'];
        ...
        $transient_key = 'construction_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $smtp_from_email = get_option('construction_smtp_from_email');
        $smtp_from_name = get_option('construction_smtp_from_name', 'Global Construction ERP');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'construction_super_admin') ? 'APPROVED' : 'PENDING';
        update_user_meta($user_id, 'construction_user_status', $status);

        // Additionally insert into custom contractor or labour tables if applicable
        global $wpdb;
        if ($role === 'construction_contractor') {
            $table_contractors = $wpdb->prefix . 'construction_contractors';
            $contractor_code = 'CON' . date('Ymd') . sprintf('%03d', rand(1, 999));
        ...
        } elseif ($role === 'construction_site_engineer') {
            $table_labours = $wpdb->prefix . 'construction_labours';
            $employee_code = 'ENG' . date('Ymd') . sprintf('%03d', rand(1, 999));
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['constsuperadmin', 'constprojectmanager', 'constsiteengineer', 'constpurchasemanager', 'constcontractor', 'constaccountant'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'construction_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'construction_super_admin', 'construction_project_manager', 'construction_site_engineer', 'construction_purchase_manager', 'construction_contractor', 'construction_accountant'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Construction ERP Portal");
        ...
        update_user_meta($user->ID, 'construction_refresh_token', $refresh_token);

        $status = get_user_meta($user->ID, 'construction_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'construction_refresh_token', true);
        ...
        update_user_meta($user_id, 'construction_refresh_token', $new_refresh);
