...
class AuthService {
    ...
    public static function logActivity(?int $user_id, string $action, string $details = '') {
        global $wpdb;
        $table = $wpdb->prefix . 'ser_activity_logs';
    ...
    public function initiateRegister(array $data) {
        ...
        $role = sanitize_text_field($data['role'] ?? 'service_technician');

        // Validate role is allowed
        $allowed_roles = ['service_super_admin', 'service_manager', 'service_technician', 'service_customer_care', 'service_accountant'];
        ...
        $transient_key = 'service_reg_otp_' . md5($email);
        $saved_data = [
            'username' => $username,
            'email' => $email,
            'password' => $password,
        ...
        $subject = get_option('service_email_subject', 'Service ERP Verification Code');
    ...
    public function register(array $data) {
        ...
        $status = ($role === 'service_super_admin') ? 'APPROVED' : 'PENDING';
        update_user_meta($user_id, 'service_user_status', $status);
    ...
    public function initiateLogin(string $username_or_email) {
        ...
        $seeded_usernames = ['ssuperadmin', 'smanager', 'stechnician', 'scustomercare', 'saccountant'];
        ...
        // Generate OTP
        $otp = strval(rand(100000, 999999));
        $transient_key = 'service_login_otp_' . md5($user->user_email);
    ...
    public function loginWithOtp(string $username_or_email, string $otp) {
        ...
        $allowed_roles = ['administrator', 'service_super_admin', 'service_manager', 'service_technician', 'service_customer_care', 'service_accountant'];
        if (!in_array($role, $allowed_roles)) {
            self::logActivity($user->ID, 'LOGIN_DENIED', "User has no authorization for Service ERP Portal");
        ...
        update_user_meta($user->ID, 'service_refresh_token', $refresh_token);

        $status = get_user_meta($user->ID, 'service_user_status', true) ?: 'APPROVED';
    ...
    public function refreshToken(string $refresh_token) {
        ...
        $stored_token = get_user_meta($user_id, 'service_refresh_token', true);
        ...
        update_user_meta($user_id, 'service_refresh_token', $new_refresh);
