...
class MediaController extends BaseController {
    ...
    public function upload(WP_REST_Request $request) {
        ...
        if (!in_array($file_ext, $allowed_exts)) {
            return $this->error('Validation failed: Supported formats are JPG, JPEG, PNG, WEBP, PDF, DOCX.');
        }
        ...
        // 4. Save metadata to wp_school_documents if related parameters exist
        $related_type = $request->get_param('related_type'); // STUDENT, TEACHER, PARENT
        $related_id = $request->get_param('related_id');
        $doc_type = $request->get_param('document_type'); // Aadhaar, Birth Certificate, PAN, etc.

        $doc_id = null;
        if (!empty($related_type) && !empty($related_id) && !empty($doc_type)) {
            $table_docs = $wpdb->prefix . 'school_documents';
            $wpdb->insert(
                $table_docs,
                [
