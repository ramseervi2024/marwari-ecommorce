...
class AuthController extends BaseController {
    ...
    public function logout(WP_REST_Request $request) {
        $user_id = get_current_user_id();
        delete_user_meta($user_id, 'realestate_refresh_token');
    ...
    public function getSmtpSettings(WP_REST_Request $request) {
        return $this->success('SMTP settings retrieved.', [
            ...
            'smtp_host' => get_option('realestate_smtp_host', ''),
            'smtp_port' => get_option('realestate_smtp_port', '587'),
            'smtp_username' => get_option('realestate_smtp_username', ''),
            'smtp_encryption' => get_option('realestate_smtp_encryption', 'tls'),
    ...
    public function saveSmtpSettings(WP_REST_Request $request) {
        ...
        update_option('realestate_smtp_enabled', sanitize_text_field($params['smtp_enabled'] ?? 'no'));
        update_option('realestate_smtp_host', sanitize_text_field($params['smtp_host'] ?? ''));
        update_option('realestate_smtp_port', sanitize_text_field($params['smtp_port'] ?? '587'));
        update_option('realestate_smtp_username', sanitize_text_field($params['smtp_username'] ?? ''));
        if (isset($params['smtp_password']) && $params['smtp_password'] !== '******') {
            update_option('realestate_smtp_password', sanitize_text_field($params['smtp_password']));
        }
        update_option('realestate_smtp_encryption', sanitize_text_field($params['smtp_encryption'] ?? 'tls'));
        update_option('realestate_smtp_from_email', sanitize_email($params['smtp_from_email'] ?? ''));
        update_option('realestate_smtp_from_name', sanitize_text_field($params['smtp_from_name'] ?? 'Real Estate ERP'));
    ...
    public function testSmtpSettings(WP_REST_Request $request) {
        ...
        $subject = 'Real Estate ERP - SMTP Connection Test';
        $message = 'This is a test email from your Real Estate CRM + ERP SMTP configuration. If you receive this, your SMTP settings are working perfectly!';
    ...
    public function getUsers(WP_REST_Request $request) {
        $users = get_users([
            'role__in' => ['realestate_super_admin', 'realestate_sales_manager', 'realestate_sales_executive', 'realestate_broker', 'realestate_accountant']
        ]);
        ...
        foreach ($users as $u) {
            $list[] = [
                ...
                'status' => get_user_meta($u->ID, 'realestate_user_status', true) ?: 'APPROVED'
            ];
        }
    ...
    public function updateUserStatus(WP_REST_Request $request) {
        ...
        update_user_meta($uid, 'realestate_user_status', $status);
